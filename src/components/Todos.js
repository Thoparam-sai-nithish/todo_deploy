...
function Todos() {
  ...
  const onSubmit = (data) => {
    ...
    console.log(data);
    axios
      .post("http://localhost:3500/todo/post", data)
  ...
  return (
    <div className="border border-2 p-2 border-success bg-dark rounded-4 todosStyles">
      ...
      >
        <h3 className=" ps-5 text-warning mb-4">
          {" "}
        ...
        {/* Select Priority */}
        <select
          className="todoPriority m-0 p-0 rounded-3 col-3"
          defaultValue={"Medium"}
        ...
        {/* Submit Button */}
        <input
          className="addTodo col-2 m-auto p-0 rounded-3 border-1"
          type="submit"
        />

        {errors.taskName && (
          <div className="text-center text-danger m-0 p-0 border mx-auto my-2">
            *Enter the task{" "}
          </div>
        )}
      </form>
      ...
      <div className="incomplete m-3 p-3 text-white rounded-4 border rounded-4   ">
        <h2 className=" ps-5 text-primary">
          <GiProgression className="mx-2 text-warning" />
          Incomplete Tasks
        </h2>
          ...
          {tasks?.map(
            (obj) =>
              obj.taskStatus === "incomplete" && (
                <tr
                  className="row m-0 bg-secondary rounded my-1 p-1 "
                  key={obj.id}
                >
      ...
      <div className="completed m-3 p-3 rounded-4  border">
        <h2 className="ps-5  ">
          <MdOutlineDoneOutline className="mx-2 " />
          Completed Tasks
        </h2>
        <table className="table text-white  p-0">
          {tasks?.map(
